...
# ── Función: medir con encoding específico → retorna "bytes tiempo encoding"
medir() {
    local recurso="$1"
    local encoding="$2"

    # Medir tamaño de descarga y tiempo total
    read -r size time <<< "$(curl -s -o /dev/null \
    ...
    echo "${size:-0} ${time:-0} ${enc_header:-none}"
}
...
declare -A D1_SIZE D1_TIME D6_SIZE D6_TIME D9_SIZE D9_TIME

for nivel in 1 6 9; do
    set_deflate_level "${nivel}"
    for recurso in "${RECURSOS[@]}"; do
        read -r sz tm enc <<< "$(medir "$recurso" "gzip")"
        case "$nivel" in
            1) D1_SIZE["$recurso"]="$sz"; D1_TIME["$recurso"]="$tm" ;;
            6) D6_SIZE["$recurso"]="$sz"; D6_TIME["$recurso"]="$tm" ;;
            9) D9_SIZE["$recurso"]="$sz"; D9_TIME["$recurso"]="$tm" ;;
        esac
        echo "    [D${nivel}] ${recurso}: ${sz}B en ${tm}s (${enc})"
    done
done
...
declare -A B5_SIZE B5_TIME B11_SIZE B11_TIME

for nivel in 5 11; do
    set_brotli_level "${nivel}"
    for recurso in "${RECURSOS[@]}"; do
        read -r sz tm enc <<< "$(medir "$recurso" "br")"
        case "$nivel" in
            5)  B5_SIZE["$recurso"]="$sz";  B5_TIME["$recurso"]="$tm"  ;;
            11) B11_SIZE["$recurso"]="$sz"; B11_TIME["$recurso"]="$tm" ;;
        esac
        echo "    [B${nivel}] ${recurso}: ${sz}B en ${tm}s (${enc})"
    done
done
...
for recurso in "${RECURSOS[@]}"; do
    ...
    d1="${D1_SIZE[$recurso]:-N/A}"; t1="${D1_TIME[$recurso]:-N/A}"
    d6="${D6_SIZE[$recurso]:-N/A}"; t6="${D6_TIME[$recurso]:-N/A}"
    d9="${D9_SIZE[$recurso]:-N/A}"; t9="${D9_TIME[$recurso]:-N/A}"
    ...
    echo "| \`${recurso}\` | ${raw} | ${d1} | ${t1} | ${d6} | ${t6} | ${d9} | ${t9} | **${ahorro}** |"
done
...
for recurso in "${RECURSOS[@]}"; do
    raw="${RAW_SIZE[$recurso]}"
    b5="${B5_SIZE[$recurso]:-N/A}";  t5="${B5_TIME[$recurso]:-N/A}"
    b11="${B11_SIZE[$recurso]:-N/A}"; t11="${B11_TIME[$recurso]:-N/A}"
    ...
    echo "| \`${recurso}\` | ${raw} | ${b5} | ${t5} | ${b11} | ${t11} | **${ahorro}** |"
done
